...
def extract_charts_from_payload(output_steps: list[dict[str, Any]]) -> list[tuple[pd.DataFrame, str]]:
    """
    ...
    if not CHART_DATA_JSON_FUNCTION:
        raise ValueError(f"Missing environment variable: CHART_DATA_JSON_FUNCTION")
    
    # Processes steps in logical order to maintain sequence
    for step in reversed(output_steps):
        if step.get("type") == "function_call_output" and CHART_DATA_JSON_FUNCTION in step.get("name"):
            try:
...
def convert_markdown_table(markdown_table: str) -> pd.DataFrame | None:
    ...
    grouped_data = [
        raw_data[i : i + col_qty] 
        for i in range(0, len(raw_data), col_qty)
    ]
