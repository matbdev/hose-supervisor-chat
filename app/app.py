...
with st.container(key='header-container'):
    ...
    with st.container(horizontal_alignment="center", width="stretch"):
        st.title("Consultor Especializado de Mangueiras", text_alignment="center", anchor=False)
...
with st.sidebar:
    ...
    if len(historico_chats) == 0:
        ...
        for chat in historico_chats:
            tipo_botao = "secondary" if st.session_state.chat_thread_id == chat["id"] else "tertiary"
            
            if st.button(chat["title"], key=f"chat_{chat['id']}", width="stretch", type=tipo_botao):
...
for idx, msg in enumerate(st.session_state.messages):
    with st.chat_message(msg["role"]):
        if msg.get("is_error"):
            ...
            if msg["role"] == "assistant" and "raw_data" in msg and "viz_list" not in msg:
                ...
                # Fixed key
                render_complex_response(text_list, charts_data=charts, msg_index=idx)
            ...
            # Render if graphics are already processed in the current session
                ...
                text_list = split_text_for_table(msg["content"])
                render_complex_response(text_list, msg_index=idx)
                
                if "viz_list" in msg:
                    for fig_idx, fig in enumerate(msg["viz_list"]):
                        st.plotly_chart(fig, key=f"hist_chart_{idx}_{fig_idx}")
            else:
...
if len(st.session_state.messages) == 0:
    with welcome_placeholder.container(key="welcome_screen"):
        ...
        with col_intro:
            ...
            Estou integrado ao Databricks para te ajudar com análises, especificações técnicas e geração de gráficos. 
            Para começar, digite sua dúvida na barra abaixo.
            """)
            
            st.markdown("💡 **Exemplos do que você pode me perguntar:**")
            st.markdown("- *Mostre as especificações da mangueira de alta pressão modelo X.*")
            st.markdown("- *Gere um gráfico com as vendas de mangueiras industriais no último trimestre.*")
            st.markdown("- *Compare as duas mangueiras mais vendidas no mês passado.*")
    ...
    with st.chat_message("assistant"):
        with st.spinner('O especialista está analisando sua pergunta...'):
            ...
            if response.status_code == 200:
                ...
                error_msg = f"Erro na comunicação com o Databricks (Status {response.status_code})."
                st.error(error_msg)
